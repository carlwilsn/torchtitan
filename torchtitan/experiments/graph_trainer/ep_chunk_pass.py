...
from torchtitan.experiments.graph_trainer.registry import register_trace_input_preparer
...
def _set_synthetic_meta(
    ...
    *,
    ...
    parent_fqn = region.root_fqn.rsplit(".", 1)[0] if "." in region.root_fqn else ""
    if parent_fqn:
...
def _pattern_root(pattern: str, fqn: str) -> str | None:
    """Return the matched root prefix for a module FQN pattern."""
...
def _region_roots(gm: fx.GraphModule, patterns: list[str]) -> list[str]:
    ...
    for node in gm.graph.nodes:
        ...
        for pattern in patterns:
            if (root := _pattern_root(pattern, fqn)) is not None:
                roots.add(root)
...
def _find_regions(
    ...
    *,
    ...
    regions = []
    for root in _region_roots(gm, patterns):
        for is_backward in (False, True):
            ...
            if nodes:
                regions.append(_Region(root, is_backward, nodes))
...
def mark_chunk_dynamic_dims(tensor: torch.Tensor, *, mode: ChunkMode) -> None:
    """Step 1: mark the user-selected tensor dimension dynamic with a hint."""
...
def prepare_ep_overlap_trace_inputs(
    ...
    if "ep_overlap" not in compile_config.passes:
        return
    mode, chunk_strategy, _ = validate_ep_overlap_config(compile_config)
...
def _chunk_symbol_subs(symbol_hints: dict[object, int]) -> dict[object, object]:
    return {symbol: symbol // 2 for symbol in symbol_hints}
...
def _rewrite_chunk_symint(
    ...
    if not chunk_symbols or not hasattr(value, "node"):
        return value
    expr = value.node.expr.subs(_chunk_symbol_subs(symbol_hints))
...
def _chunk_tensor_meta(
    ...
    if not depends_on_chunk_symbol(val, symbol_hints):
        return val
    shape = tuple(_rewrite_chunk_symint(dim, symbol_hints) for dim in val.shape)
    stride = tuple(_rewrite_chunk_symint(dim, symbol_hints) for dim in val.stride())
    return val.new_empty_strided(shape, stride)


def _chunk_meta_value(value: object, symbol_hints: dict[object, int]) -> object:
    if isinstance(value, torch.Tensor):
        return _chunk_tensor_meta(value, symbol_hints)
    if isinstance(value, (torch.SymInt, torch.SymFloat, torch.SymBool)):
        return _rewrite_chunk_symint(value, symbol_hints)
    return value
...
def _chunk_copied_meta(
    ...
    if "val" in copied:
        copied["val"] = tree_map(
            lambda value: _chunk_meta_value(value, symbol_hints), copied["val"]
        )
    return copied
...
def _map_arg(
    ...
    *,
    ...
    if len(symbols) == 1:
        ...
        if str(expr) == str(symbol) and expr_key(expr) in scalars:
...
def _symbolic_expr(value: object) -> object:
...
def _same_symbolic_expr(lhs: object, rhs: object) -> bool:
    return str(_symbolic_expr(lhs)) == str(_symbolic_expr(rhs))
...
def _is_contiguous_chunk_input_stride(
    ...
    for live_in in split_live_ins:
        ...
        if any(_same_symbolic_expr(value, stride) for stride in chunk_val.stride()):
            return True
    return False
    ...
    *,
    ...
    for node in plan.region.nodes:
        ...
        if (
            ...
            and _mutates_input(node)
            and node in prior_chunk_copies
            and isinstance(args, tuple)
            and args
            and isinstance(args[0], fx.Node)
            and args[0] not in copied
            and isinstance(new_args, tuple)
            and new_args
        ):
            # Match eager chunking for shared mutable live-ins such as MoE
            # token-count buffers: the later chunk must consume the earlier
            # chunk's mutation result, making the alias dependency explicit.
            new_args = (prior_chunk_copies[node], *new_args[1:])
        if in_place and node not in preserve_originals:
            new_node = node
            new_node.args = new_args
            new_node.kwargs = new_kwargs
        else:
            with gm.graph.inserting_before(node.next):
                new_node = gm.graph.create_node(
                    node.op, node.target, new_args, new_kwargs, type_expr=node.type
                )
        new_node._rename(f"{name}_chunk{chunk_id}")
        new_node.meta = _chunk_copied_meta(meta, symbol_hints)
        _set_chunk_meta(
            new_node,
            region=plan.region,
            role="body",
    ...
    *,
    ...
    for live_in in sorted(split_live_ins, key=order.get):
        ...
        with gm.graph.inserting_before(first_chunk_symbol_consumer(live_in)):
            size = gm.graph.call_function(aten.sym_size.int, args=(live_in, dim))
            _set_synthetic_meta(size, region=plan.region, role="split_boundary")
            size.meta["val"] = val.shape[dim]
            ...
            symbols = dynamic_dim_symbols(val, dim)
        ...
        if (
            ...
            with gm.graph.inserting_after(chunks[source][1]):
                for chunk_id, chunk in enumerate(chunks[source]):
                    prop = gm.graph.call_function(
                        live_in.target, args=(chunk, prop_dim)
                    )
                    _set_synthetic_meta(
                        prop,
                        region=plan.region,
...
def _ep_roots(gm: fx.GraphModule, module_pattern: str) -> list[str]:
    ...
    for node in gm.graph.nodes:
        ...
        if not fqn:
            continue
        root = _pattern_root(module_pattern, fqn)
        if root is not None:
            roots.add(root)
    return sorted(roots)
