...
2. Symbol helpers treat the unbacked SymInt selected by mark_dynamic as the
...
def dynamic_dim_symbols(val: torch.Tensor, dim: int) -> frozenset[object]:
    """Return the symbols present in ``val.shape[dim]``."""
    return free_symbols(val.shape[dim])
...
def expr_key(value: object) -> str:
    """Return a stable string key for concrete or symbolic expressions."""
    return str(value.node.expr if isinstance(value, torch.SymInt) else value)
...
def _validate_no_chunk_symbols(
    ...
    for node in gm.graph.nodes:
        for value in tree_leaves((node.args, node.kwargs, node.meta.get("val"))):
            if depends_on_chunk_symbol(value, symbol_hints):
                raise ValueError(
                    f"concretize_ep_chunk_symbolic_shapes_pass left chunk symbol(s) "
                    f"{free_symbols(value) & chunk_symbols} in {node.name}."
                )
...
def _is_dead_chunk_scalar_plumbing(node: fx.Node) -> bool:
    ...
    return (
        ...
        and not node.users
        and (
            node.target is aten.sym_size.int
            or node.target is torch.sym_ite
            or node.target
            in (
                operator.and_,
                operator.floordiv,
            ...
            or "assert" in getattr(node.target, "__name__", "")
        )
    )
...
def concretize_ep_chunk_symbolic_shapes_pass(
    ...
    for node in list(gm.graph.nodes):
        if node.op == "output":
            continue
        node.args = tree_map(lambda arg: _concretize_arg(arg, symbol_hints), node.args)
        node.kwargs = tree_map(
            lambda arg: _concretize_arg(arg, symbol_hints), node.kwargs
        )
        val = node.meta.get("val")
        if val is not None:
            new_val = tree_map(
    ...
    if scalar_constants:
        logger.debug(
            ...
            len(scalar_constants),
        )
        for node in gm.graph.nodes:

            def replace_scalar(arg):
                if arg in scalar_constants:
                    return scalar_constants[arg]
                return arg
